...
Quotes and intents have `expires_at` and `purge_at`. API access opportunistically purges due rows;
pg-boss also schedules an hourly purge. Provider events are removed before intents, then
unreferenced quotes. Development defaults retain an expired intent for one day and an expired
quote for one hour.

...
4. Reconcile affected opaque provider references before communicating status.

## Database down

1. Confirm the database health check and storage availability.
2. Stop creating new provider intents if durable writes cannot be guaranteed.
...
1. Stop creating intents for the affected direction through a reviewed control outside this
   read-only dashboard.
2. Preserve opaque provider references and normalized timestamps.
3. Ask the provider to reconcile collection, settlement and refunds.
4. Resume only after provider health and reconciliation agree.
...
1. Confirm count and oldest age; provider events are normalized but currently unprocessed by
   design.
2. Compare opaque event IDs with provider reporting outside metrics.
...
4. Reconcile unknown provider outcomes before retrying.
...
2. Compare normalized Ntumba state with provider evidence.
...
1. Confirm the provider owns and has initiated the refund.
2. Track the opaque provider reference outside metrics under restricted access.
3. Do not issue or front the refund from Ntumba funds; Ntumba has no treasury or balance.
4. Communicate only provider-confirmed status and escalate breached provider timelines.
